# S2-PMTiles Version 1 Specification
...
S2-PMTiles is a single-file archive format for tiled data that is backwards compatible with [PMTiles v3](https://github.com/protomaps/PMTiles/blob/main/spec/v3/spec.md).

The recommended MIME Type for S2-PMTiles is `application/vnd.s2pmtiles`.

## 2 Overview

A S2-PMTiles archive consists of five main sections:
...
The Header has a length of 262 bytes and is always at the start of the archive. It includes everything needed to decode the rest of the S2-PMTiles archive properly.
...
The Leaf Directories Length is an 8-byte field specifying the accumulated size (in bytes) of all leaf directories. A value of `0` indicates that there are no leaf directories included in this S2-PMTiles archive.
...
The Number of Addressed Tiles is an 8-byte field specifying the total number of tiles in the S2-PMTiles archive, before RunLength Encoding.
...
> NOTE:
> This feature is **deprecated** in this spec. IC MUST always be `0x01` (None). Kept for compatibility purposes, but the s2-pmtiles parsers should fail if IC is not `0x01` with pmtiles data.
...
The Center Zoom is a 1-byte field specifying the center zoom (LOD) of the tiles. A reader MAY use this as the initial zoom when displaying tiles from the S2-PMTiles archive.

This field is encoded as an 8-bit unsigned integer.

#### Center Position

The Center Position is an 8-byte field that includes the latitude and longitude of the center position. A reader MAY use this as the initial center position when displaying tiles from the S2-PMTiles archive.
